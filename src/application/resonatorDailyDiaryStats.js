...
export async function getDailyDiaryData({ resonator_id }) {
    ...
    const [dailyDiary_data] = await Promise.all([
        getQuestionFromSheet1(sheet_id)
    ]);
    ...
    return {
        ...
        diary_data: dailyDiary_data
    };
}

export async function sendDailyDiaryAnswer({ resonator_id, question_id, answer }) {
    const [resonator] = await Promise.all([
        resonatorRepository.findById(resonator_id)
    ]);
    ...
    await Promise.all([
        saveAswerToSheet1(sheet_id, question_id, answer)
    ]);
    await getUow().commit();
...
async function saveAswerToSheet1(sheet_id, q_id, answer) {
    if (answer) {
...
async function getQuestionFromSheet1(sheet_id) {
